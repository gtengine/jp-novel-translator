...
def save_syosetu_novel(novel_url, save_path):
    ...
    for episode in tqdm(episodes):
        ...
        for line in lines:
            new_line = line.text.replace('「', '"').replace('」', '"').replace('『', '"').replace('』', '"')
            if line.find('ruby'):
                new_line = new_line.replace(line.find('rb').text, f"{line.find('rb').text}({line.find('rt').text})")
        ...
        try:
            ...
            if ep_view:
                ...
                for view_line in view_lines:
                    new_view_line = view_line.replace('「', '"').replace('」', '"').replace('『', '"').replace('』', '"')
...
def save_kakuyomu_novel(novel_url, save_path):
    ...
    for episode in tqdm(episodes):
        ...
        lines = contents.find_all('p')
        for line in lines:
            new_line = line.text.replace('「', '"').replace('」', '"').replace('・', '-').replace('『', '"').replace('』', '"')
            texts.append(f'{new_line}\n')
        texts.append(f"{'====='*4}\n")
